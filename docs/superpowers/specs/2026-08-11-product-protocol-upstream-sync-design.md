...
- **Mirror 267/268:** `default_tile_count: 1`, `tile_width: 5`, `tile_height: 10`,
  `uplight_zone_count: 25` (rear), firmware per LIFX (TBD — use a sane matrix default,
  flag for confirmation), notes describing the front/rear split.
- **Ceiling 265/266:** dimensions matching the new variant; `uplight_zone_count: 1`;
  firmware 4.x.
- **Existing Ceiling (176/177 8×8, 201/202 16×8):** add `uplight_zone_count: 1` so the
  split is modelled uniformly.
- Fill any other of the 15 new products that fall in scope (matrix/multizone dimensions);
  leave non-matrix new bulbs at catalogue defaults.
- Extend `SpecsRegistry`/`ProductSpec` (`products/specs.py`) with the new
  `uplight_zone_count` field (optional, `int | None`).
...
- `DeviceState` (`devices/states.py`): ensure `has_buttons` drives button state, add
  ambient-light sensor state (configurable lux) and button config/backlight state,
  add uplight/downlight helper properties derived from specs.
...
- New `handlers/sensor_handlers.py`: `SensorGetAmbientLight (401)` →
  `SensorStateAmbientLight (402)` returning the device's configured lux. Register in
...
- Handlers: Sensor ambient-light response; Button get/set/config; `has_buttons` gating;
  StateUnhandled for non-capable devices.
