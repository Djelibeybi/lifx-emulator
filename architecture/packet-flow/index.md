...
1. Header is parsed to determine target device(s)
1. Acknowledgment sent immediately (unless a scenario targets acks)
1. Payload is unpacked into a typed packet object
1. Device processes the packet through its handler registry
1. Scenarios are applied (drops, delays, malformed responses)
1. Response packets are packed and sent back via UDP
...
- [Architecture Overview](https://djelibeybi.github.io/lifx-emulator/architecture/overview/index.md) — High-level component diagram
- [Protocol Layer](https://djelibeybi.github.io/lifx-emulator/architecture/protocol/index.md) — Header and packet structure details
- [Device State](https://djelibeybi.github.io/lifx-emulator/architecture/device-state/index.md) — How state is read and updated by handlers
