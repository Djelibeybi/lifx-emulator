...
- [ ] **MDNS-03**: Each advertised device produces its own reply packet containing a PTR record `_lifx._udp.local` → `<serial>._lifx._udp.local`, an SRV record (priority 0, weight 0, the emulator's UDP port, a per-device `.local` hostname), a TXT record and one address record; a discovery query produces one reply packet per advertised device
